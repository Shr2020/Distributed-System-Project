...
#------ R and w fixed to 1
  ...
  test "test k version staleness" do
    ...
    view = [:a, :b, :c,:d,:e,:f,:g]
    base_config =
      Dynamo.new_configuration(view, 1, 1, 50_000, 70_000)
    ...
    client =
      spawn(:client, fn ->
        client = Dynamo.Client.new_client(:client)
       measure_k_staleness(view,client) 
  ...
  def measure_k_staleness(view,client) do
      ...
      value_list = write_staleness_reads_probability(view,client,[],100)
          for x <- 1..100 do
           ...
           newlist = splitlist(value_list,8,[])
  ...
  receive do
      {sender,{:get,^k,v}} ->
        ...
        if count > 1 do
          ...
          IO.puts("probability is  #{inspect(updated/100)}\n")
